...
<body>
        ...
        <div class="content-wrap">
            ...
            <div class="card">
                ...
                <form id="addForm" style="display: flex; flex-direction: column; gap: 16px;">
                    <div style="display: flex; gap: 12px; flex-wrap: wrap;">
                        ...
                        <select id="pathMode" style="padding: 14px 16px; border: 1px solid var(--border-default); border-radius: var(--radius-md); background:var(--bg-input); flex: 1;">
                            <option value="auto">🔀 路径（自动）</option>
                            <option value="emby">📌 路径（强制）</option>
                            <option value="none">🚫 路径（无）</option>
                        </select>
                    </div>
                    ...
                    <div style="display: flex; gap: 12px; flex-wrap: wrap; align-items: flex-start;">
                        <div style="position: relative; flex: 2; min-width: 200px;">
                            <div style="display:flex; gap:12px; align-items:center; margin-bottom: 8px;">
                                <div style="display:flex; gap:12px; align-items:center; background:var(--bg-card); padding:12px 16px; border-radius:var(--radius-md); border:1px solid var(--border-default); cursor: pointer; transition:0.2s; flex: 1;" onclick="toggleIconPicker(event)" id="iconSelectBtn">
                                ...
                                <input type="text" id="iconSearch" placeholder="🔍 搜索图标..." style="width: 120px; padding: 10px 14px; border: 1px solid var(--border-default); border-radius: var(--radius-md); background:var(--bg-input); font-size: 14px;" onkeyup="filterIcons()">
                            </div>

                            <div id="iconPickerPanel" style="display:none; position: absolute; top: 100%; left: 0; min-width: 320px; width: auto; background: var(--bg-card); border: 1px solid var(--border-default); border-radius: var(--radius-md); padding: 12px; box-shadow: var(--shadow-lg); z-index: 100; margin-top: 4px;">
                                <div style="display: flex; gap: 6px; align-items: center; margin-bottom: 8px; flex-wrap: nowrap;">
                                    <input type="text" id="customIconUrlInput" placeholder="图标库链接..." style="flex: 1; padding: 8px 10px; border: 1px solid var(--border-default); border-radius: var(--radius-sm); background:var(--bg-input); font-size: 12px;">
                                    <button type="button" onclick="setCustomIconLibrary()" class="btn-submit" style="padding: 8px 10px; font-size: 12px; white-space: nowrap;">加载</button>
                                    <button type="button" onclick="resetIconLibrary()" style="padding: 8px 10px; background: var(--text-muted); color: white; border: none; border-radius: var(--radius-sm); cursor: pointer; font-size: 12px; white-space: nowrap;">默认</button>
                                </div>
                                <div id="iconGrid" style="overflow: auto; max-height: 240px; padding: 4px; display: grid; grid-template-columns: repeat(6, 52px); gap: 6px;">
            ...
            <div class="card">
                ...
                <div style="background: var(--info-bg); padding: 16px 20px; border-radius: var(--radius-md); border: 1px dashed rgba(59, 130, 246, 0.3); margin-bottom: 20px; display: flex; align-items: center; gap: 16px; flex-wrap: wrap;">
                    ...
                    <select id="batch-pathmode-select" style="padding: 8px 12px; border-radius: var(--radius-sm); border: 1px solid var(--border-default); background: var(--bg-input); color: var(--text-primary); font-weight: 600;">
                        <option value="auto">🔀 路径（自动）</option>
                        <option value="emby">📌 路径（强制）</option>
                        <option value="none">🚫 路径（无）</option>
                    </select>
    ...
    <script>
                        ...
                        <div class="card-details" style="display: none; padding-top: 12px; border-top: 1px solid var(--border);">
                            <div style="display: flex; flex-direction: column; gap: 10px;">
                                ...
                                <div class="info-row">
                                    ...
                                    <span class="info-label" style="margin-left: 16px;">路径模式:</span>
                                    <span style="font-weight:600;">\${r.path_mode === 'emby' ? '📌 /emby' : r.path_mode === 'none' ? '🚫 无前缀' : '🔀 自动'}</span>
                                </div>
                            </div>

                            <div class="card-footer" style="margin-top: 16px;">
                                <button class="btn-edit" onclick="editNode('\${r.prefix}', '\${r.target}', '\${r.mode}', '\${r.remark || ''}', '\${r.icon || ''}', '\${r.cache_img}', '\${r.path_mode || 'auto'}')">编辑配置</button>
        ...
        function editNode(prefix, targetStr, mode, remark, icon, cacheImg, pathMode) {
            document.getElementById('oldPrefix').value = prefix;
            document.getElementById('remark').value = remark;
            document.getElementById('prefix').value = prefix;
            document.getElementById('mode').value = mode || 'off';
            document.getElementById('pathMode').value = pathMode || 'auto';
        ...
        document.getElementById('addForm').onsubmit = async (e) => {
            ...
            const path_mode = document.getElementById('pathMode').value;
            ...
            try {
                const res = await fetch('/api/routes', { 
                    method: 'POST', 
                    body: JSON.stringify({oldPrefix, prefix, target, mode, path_mode, remark, icon, cache_img})
                });
                const data = await res.json();
                ...
                document.getElementById('pathMode').value = 'auto';
    ...
    // 🚀 魔法功能：自动继承现有的模式选项 (增强稳定版)
        ...
        async function batchUpdateModes() {
            ...
            const newPathMode = document.getElementById('batch-pathmode-select').value;
            ...
            if (!newMode && !newPathMode) {
                statusElem.innerText = "⚠️ 请先选择要修改的模式或路径模式！";
                statusElem.style.color = "#ff9500";
                return;
            }
            ...
            try {
                // 构建更新数据
                const updateData = { prefixes: selectedPrefixes };
                if (newMode) updateData.mode = newMode;
                if (newPathMode) updateData.path_mode = newPathMode;
...
async function sendTgNodeStatus(env, chatId, messageId = null) {
    try {
        ...
        const routes = await env.DB.prepare(`SELECT prefix, remark, target, mode, last_play, path_mode FROM routes ORDER BY sort_order ASC, prefix ASC`).all();
...
export default {
    ...
    async fetch(request, env, ctx) {
            ...
            try {
                ...
                for (const r of routes) {
                    if (r.prefix && r.target) {
                        await env.DB.prepare('INSERT OR REPLACE INTO routes (prefix, target, mode, remark, last_play, icon, cache_img, sort_order, path_mode) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)')
                            .bind(r.prefix, r.target, r.mode || 'off', r.remark || '', r.last_play || '', r.icon || '', r.cache_img || 'on', r.sort_order || 0, r.path_mode || 'auto').run();
                    }
                }
                return Response.json({ success: true });
            } catch (e) { return Response.json({ success: false, error: e.message }); }
        ...
        if (url.pathname.startsWith('/api/routes')) {
            ...
            try { await env.DB.exec(`ALTER TABLE routes ADD COLUMN path_mode TEXT DEFAULT 'auto'`); } catch(e) {}
            ...
            if (request.method === 'POST') {
                ...
                await env.DB.prepare('INSERT OR REPLACE INTO routes (prefix, target, mode, remark, icon, cache_img, sort_order, path_mode) VALUES (?, ?, ?, ?, ?, ?, ?, ?)')
                    .bind(data.prefix, data.target, data.mode || 'off', data.remark || '', data.icon || '', data.cache_img || 'on', currentSortOrder, data.path_mode || 'auto').run();
                return Response.json({ success: true });
            }
            ...
            if (request.method === 'PUT') {
                ...
                if (data.prefixes && Array.isArray(data.prefixes) && (data.mode !== undefined || data.path_mode !== undefined)) {
                    ...
                    if (data.path_mode !== undefined) {
                        updates.push('path_mode = ?');
                        values.push(data.path_mode);
                    }
                    await env.DB.prepare(`UPDATE routes SET ${updates.join(', ')} WHERE prefix IN (${placeholders})`)
                        .bind(...values, ...data.prefixes).run();
                    return Response.json({ success: true });
                }
        ...
        // 2.6 核心反代与调度引擎
        // ==========================================
        let targetUrls = []; let currentMode = 'off'; let enableCache = true; let remainingPath = ''; let currentPathMode = 'auto';
        ...
        //
        // path_mode 选项：
        // - 'auto'  : 默认行为，根路径失败后自动回退 /emby
        // - 'emby'  : 始终强制添加 /emby 前缀（给分离部署但裸域名的节点用）
        // - 'none'  : 完全不加 /emby 前缀（给普通 Emby 根路径部署用）
        function buildUpstreamCandidates(targetBase, remainingPath, search, pathMode = 'auto') {
            const candidates = [];
            ...
            // 'none' 模式：不做任何回退，直接返回原地址
            if (pathMode === 'none') return candidates;
            ...
            try {
                ...
                // 'emby' 模式：强制添加 /emby；'auto' 模式：只在需要时回退
                const shouldTryEmby = (pathMode === 'emby') || (pathMode === 'auto' && hasNoBasePath && !requestAlreadyHasEmbyPrefix && isLikelyMediaServerPath(requestPath));
        ...
        // PlaybackInfo 里的媒体地址改写函数。
        // 之前这里只处理了以 http/https 开头的绝对地址，普通 Emby 返回绝对地址时可以正常播放，
        // 但 HUD 返回 /emby/videos/... 这类根路径相对地址时，客户端会直接请求面板根路径，
        // 请求不会再经过 /{prefix}/... 这条代理路由，最终就表现为 PlaybackInfo 成功、真正播放失败。
        // 该函数完整处理 4 种地址格式：
        // 1. 绝对地址: https://...         -> 追加 Worker 前缀
        // 2. 协议相对: //v1.uhdnow.com/...  -> 补上当前线路前缀
        // 3. 根路径相对: /emby/videos/...  -> 保留路径并补上当前线路前缀
        // 4. 普通相对路径: videos/...     -> 解析成源站绝对地址后再补线路前缀
        //
        // 这样无论 HUD 返回哪一种格式，客户端最终拿到的播放地址都会稳定落到当前 Worker 线路下。
        function rewritePlaybackMediaUrl(rawValue, targetUrl, proxyOrigin, safePrefix) {
            if (typeof rawValue !== 'string') return rawValue;
            const trimmedValue = rawValue.trim();
            if (!trimmedValue) return rawValue;

            // 避免重复改写。某些客户端会复用上一次的 PlaybackInfo，
            // 如果返回值本身已经是当前 Worker 前缀，就原样返回，防止出现 /prefix/https://... 这类坏地址。
            if (safePrefix && trimmedValue.startsWith(proxyOrigin + safePrefix + '/')) return rawValue;
            if (safePrefix && trimmedValue.startsWith(safePrefix + '/')) return proxyOrigin + trimmedValue;

            // 绝对地址和协议相对地址需要继续走"编码后的绝对 URL 反代"这条现有能力，
            // 因为这样可以完整保留源站 host、query、hash，避免多节点和跳转场景下丢信息。
            if (/^https?:\/\//i.test(trimmedValue)) return `${proxyOrigin}${safePrefix}/${trimmedValue}`;
            if (trimmedValue.startsWith('//')) return `${proxyOrigin}${safePrefix}/${targetUrl.protocol}${trimmedValue}`;

            // HUD 最容易踩坑的是这里：返回 /emby/videos/... 时，必须显式补上线路前缀。
            // 否则客户端会去请求站点根路径 /emby/...，直接绕开当前 /{prefix} 代理节点。
            if (trimmedValue.startsWith('/')) return `${proxyOrigin}${safePrefix}${trimmedValue}`;

            // 少数实现会返回不带前导斜杠的相对路径，例如 videos/123/master.m3u8。
            // 这时先按源站地址解析成绝对 URL，再映射回 Worker 代理地址，保证路径语义不变。
            try {
                const resolved = new URL(trimmedValue, targetUrl.origin + '/').href;
                return `${proxyOrigin}${safePrefix}/${resolved}`;
            } catch (e) {
                return rawValue;
            }
        }
        ...
        if (decodedPath.startsWith('/http://') || decodedPath.startsWith('/https://')) {
            ...
            try {
                ...
                const stmt = env.DB.prepare(`SELECT target, mode, cache_img, path_mode FROM routes WHERE prefix = ?`);
                const route = await stmt.bind(prefix).first();
                if (!route) return new Response(`404: Node not found`, { status: 404 });

                currentMode = route.mode || 'off'; enableCache = (route.cache_img !== 'off'); currentPathMode = route.path_mode || 'auto';
        ...
        const needsReplayableBody = canHaveRequestBody && (
            targetUrls.length > 1
            || targetUrls.some(target => buildUpstreamCandidates(target, remainingPath, url.search, currentPathMode).length > 1)
        );
        ...
        for (let i = 0; i < targetUrls.length; i++) {
            const candidateUrls = buildUpstreamCandidates(targetUrls[i], remainingPath, url.search, currentPathMode);
                ...
                if (data && data.MediaSources) {
                    data.MediaSources.forEach(source => {
                        ...
                        ['DirectStreamUrl', 'TranscodingUrl', 'Url'].forEach(key => {
                            if (source[key]) {
                                const rewritten = rewritePlaybackMediaUrl(source[key], finalTargetUrl, proxyOrigin, safePrefix);
                        ...
                        if (source.MediaStreams) {
                            source.MediaStreams.forEach(stream => {
                                if (stream?.DeliveryUrl) {
                                    const rewritten = rewritePlaybackMediaUrl(stream.DeliveryUrl, finalTargetUrl, proxyOrigin, safePrefix);
