// EMBY-PROXY-UI V18.6 (SaaS UI Optimized - Ultimate Fix + Media Auth Compatibility)
...
function shouldForwardRealClientIp(node, currentConfig = null) {
  const nodeMode = normalizeNodeRealClientIpMode(node?.realClientIpMode);
  if (nodeMode === "forward") return true;
  if (nodeMode === "strip") return false;
  return currentConfig?.forwardRealClientIp !== false;
}
...
const Proxy = {
  ...
  async buildProxyRequestState(request, node, proxyPath, requestUrl, clientIp, requestTraits, forceH1, targetBases, currentConfig = null) {
    ...
    if (shouldForwardRealClientIp(node, currentConfig)) {
              ...
              <div v-if="App.isSettingsTabVisible('proxy')" id="set-proxy" v-show="App.activeSettingsTab === 'proxy'" class="space-y-4">
                <div class="grid gap-4">
                  ...
                  <div v-if="App.isSettingsExpertMode()" class="rounded-3xl border border-slate-200 dark:border-slate-800 bg-slate-50/70 dark:bg-slate-950/40 p-5 shadow-sm settings-block h-full">
                    ...
                    <label class="flex items-center text-sm font-medium mb-2 cursor-pointer text-slate-900 dark:text-white"><input type="checkbox" id="cfg-forward-real-client-ip" v-model="App.settingsForm.forwardRealClientIp" class="mr-2 w-4 h-4 rounded" checked> 默认透传真实客户端 IP</label>
                    <p class="text-xs text-slate-500 mb-3">开启后回源时补写 <code>X-Real-IP</code> / <code>X-Forwarded-For</code>；若个别上游会按真实出口 IP、地区或 ASN 风控，可在节点里改为“强制关闭透传”。</p>
	     ...
	     <form @submit.prevent="App.saveNode()" novalidate class="space-y-4 max-h-[calc(80vh-env(safe-area-inset-bottom)-env(safe-area-inset-top))] overflow-y-auto pb-[env(safe-area-inset-bottom)] pl-[env(safe-area-inset-left)] pr-[max(0.5rem,env(safe-area-inset-right))]">
          <div v-if="App.nodeModalFeedback.message" class="rounded-2xl border px-4 py-3 text-sm font-medium" :class="App.getNodeModalFeedbackClass(App.nodeModalFeedback.tone)">
            ...
            <div>
              ...
              <p class="mt-1 text-xs text-slate-400">用于兼容不同上游的登录与会话请求。自动模式会按请求里已有的媒体认证头家族做规范化。</p>
            </div>
            <div>
              ...
              <select id="form-real-client-ip-mode" v-model="App.nodeModalForm.realClientIpMode" class="w-full px-4 py-2 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 outline-none text-sm text-slate-900 dark:text-white">
                <option value="inherit">继承全局 (推荐)</option>
                <option value="forward">强制透传</option>
                <option value="strip">强制关闭透传</option>
              </select>
              <p class="mt-1 text-xs text-slate-400">用于处理个别上游按 <code>X-Real-IP</code> / <code>X-Forwarded-For</code> 风控导致的 403。</p>
            </div>
          </div>
  ...
  <script>
    ...
    const CONFIG_FORM_BINDINGS = {
      ...
      proxy: [
        ...
        { key: 'forwardRealClientIp', id: 'cfg-forward-real-client-ip', kind: 'checkbox', checkboxMode: 'defaultTrue' },
    ...
    const CONFIG_FIELD_LABELS = {
      ...
      forwardRealClientIp: '真实客户端 IP 透传',
    ...
    Object.assign(UiBridge, {
      ...
      async loadSettings() {
          ...
          const cfg = configRes.config || { enableH2: false, enableH3: false, peakDowngrade: true, protocolFallback: true, sourceSameOriginProxy: true, forceExternalProxy: true, forwardRealClientIp: true };
